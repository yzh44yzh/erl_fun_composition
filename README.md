...
Сейчас будет немного Haskell. Дело в том, что Haskell может показать нужные нам идеи в эталонном виде. А в Erlang мы может реализовать только что-то похожее, с некоторым приближением. Так что прежде, чем смотреть на искаженную копию, сперва посмотрим на оригинал.
...
Решение на Haskell выглядит так: [Main4.hs](./hs_src/Main4.hs).
...
Сама по себе свертка реализуется тривиально:
```
bind(Arg, Funs) ->
    lists:foldl(
        fun
...
Отдельно посмотрим на валидацию списка книг. Прогнав книги через *books_shop:validate_book/2* мы получим:
```
...
Кстати, по такому же принципу устроен фреймворк Elixir Phoenix.
...
Варианты 1-й (код не лучшего качества) и 5-й (оверинжениринг) -- это примеры того, как делать не надо.
