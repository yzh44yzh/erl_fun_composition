...
То есть, у нас есть 3 функции, которые могут вернуть успешный результат, либо ошибку. Четвертая, такая же, функция, которую нужно применить несколько раз к элементам списка. И, наконец, пятая функция, которая всегда возвращает успешный результат.
...
dialyzer по-прежнему контролирует правильность композиции. Но за правильностью использования State разработчику придется следить самому. Появляются возможности для ошибок.

Кроме того функции похожи -- они повторяют одинаковый шаблон. И это наводит на мысль, что тут можно что-то обобщить, и сократить количество кода.
...
Получилось очень просто, очень лаконично. Код пишем только для happy path. try..catch решает все остальные проблемы. Красота!

В таком простом примере это решение может показаться самым лучшим. Но в больших проектах с исключениями есть некоторые проблемы.
...
Во-вторых, вызывая функцию, разработчик не может знать всех возможных вариантов ее завершения. Разве что прочитает ее код и проследит все внутренние вызовы на всю глубину. В решениях 1 и 2 все возможные варианты описаны в спецификации функции. По хорошему, спецификация могла бы содержать инфу, какие исключения могут возникнуть (как в Java). Но в Erlang этого нет.

В использовании исключений ничего плохого нет (даже если некоторые ФП программисты будут говорить вам обратное). Но в мире Erlang так сложилось, что исключения используются редко.
